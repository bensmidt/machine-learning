...
def count_tensor_elements(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  elem_ls = list(x.shape)
  print(elem_ls)
  num_elements = 1
  for i in range(len(elem_ls)):
    num_elements *= elem_ls[i]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_elements
...
def multiples_of_ten(start, stop):
  ...
  if (stop - start) < 10:
    ...
    return x
  x = torch.arange(start = start, end = stop+1, step = 10, dtype = torch.float64)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def count_negative_entries(x):
  ...
  # Replace "pass" statement with your code
  mask = x < 0
  num_neg = count_tensor_elements(x[mask])
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_neg
...
def reshape_practice(x):
  ...
  # Replace "pass" statement with your code
  a = x.view(2, 3, 4)
  y = a.transpose(0, 1).contiguous().view(3, 8)
  # print(a)
  # print(y)
  #############################################################################
  #                            END OF YOUR CODE                               #
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop: 
    tensor_ls = []
    for i in range(B): 
      tensor_ls.append(torch.mm(x[i], y[i]))
    z = torch.stack((tensor_ls), dim = 0)
...
def normalize_columns(x):
  ...
  x.t()
  M, N = x.shape
  means = ( torch.matmul( x.t(), torch.ones(N) ) ) / M
  std = torch.sqrt( torch.matmul( ((x - means)**2).t(), torch.ones(N) ) / (M - 1) )
  y = (x - means.t()) / std
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
