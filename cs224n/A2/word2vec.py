...
def oneDimSoftmax(x): 
    ...
    logC = -np.max(x)
    return np.exp(x + logC) / np.sum(np.exp(x + logC))
...
def naiveSoftmaxLossAndGradient(
    ...
    # I wrote my own softmax implementation for fun :) (see function for how it was done)
    yHat = oneDimSoftmax(outsideVectors @ centerWordVec) 
    loss = -np.log(yHat[outsideWordIdx])
    gradCenterVec = outsideVectors.T @ yHat - outsideVectors[outsideWordIdx]
...
def negSamplingLossAndGradient(
    ...
    Uo[0] *= -1
    sigmoidVec = sigmoid(Uo @ centerWordVec)

    loss = np.sum( -np.log(sigmoidVec) )
    
    gradCenterVec = (sigmoidVec - 1) @ Uo
    ...
    for i in range(len(indices)): gradOutsideVecs[indices[i]] += gradUo[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in range(len(outsideWords)): 
        ...
        curLoss, curGradCenterVecs, curGradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += curLoss
        gradCenterVecs[centerWordIdx] += curGradCenterVecs
        gradOutsideVecs += curGradOutsideVecs
