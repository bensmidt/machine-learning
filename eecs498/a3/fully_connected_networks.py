...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # scores variable.                                          #
        #############################################################
        H, hidden_cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, scores_cache = Linear.forward(H, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dH, dW2, db2 = Linear.backward(dscores, scores_cache)
        dX, dW1, db1 = Linear_ReLU.backward(dH, hidden_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          ...
          self.params["W{}".format(i+1)] = weight_scale * torch.randn(
            (in_dim, out_dim), dtype=dtype, device=device
          )
          self.params["b{}".format(i+1)] = torch.zeros((out_dim), dtype=dtype, device=device)
          in_dim = out_dim
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
          out, linear_cache = Linear_ReLU.forward(
            in_, self.params["W{}".format(i)], self.params["b{}".format(i)]
          )
          if self.use_dropout:
            dropout_in = out
            out, dropout_cache = Dropout.forward(dropout_in, self.dropout_param)
          else:
            dropout_cache = None
          caches += [(linear_cache, dropout_cache)]
          in_ = out
        ...
        for i in range(self.num_layers - 1, 0, -1):
          dout = din
          linear_cache, dropout_cache = caches[i-1]
          if self.use_dropout:
            dropout_dout = Dropout.backward(dout, dropout_cache)
            dout = dropout_dout
          din, dW, db = Linear_ReLU.backward(dout, linear_cache)
