...
def initialize_three_layer_conv_part4():
  ...
  # Hidden layer channel and kernel sizess
  channel_1 = 32
  channel_2 = 16
  kernel_size_1 = 5
  pad_size_1 = 2
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    self.cnn = nn.Sequential(ResNetStem(), 
        ResNetStage(*stage_args[0], block=block),
        ResNetStage(*stage_args[1], block=block),
        ResNetStage(*stage_args[2], block=block)
    )
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
  ...
  def forward(self, x):
    ...
    ############################################################################
    out = self.cnn(x)
    avg_pool = nn.AvgPool2d( kernel_size = (out.shape[2], out.shape[3]) )
    out = avg_pool(out)
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(nn.BatchNorm2d(Cin), nn.ReLU(), 
                              nn.Conv2d(Cin, Cout//4, 1, stride),
                              nn.BatchNorm2d(Cout//4), nn.ReLU(),
