...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train): 
    x_train_vec = x_train_flat[i]
    for j in range(num_test): 
      temp_dist = torch.sum( (x_train_vec - x_test_flat[j])**2 )
      dists[i, j] = temp_dist
  '''
  C, H, W = x_train.shape[1:]
  x_train = x_train.reshape(num_train, C*H*W)
  x_test = x_test.reshape(num_test, C*H*W)
  for i, train_i in enumerate(x_train):
    for j, test_j in enumerate (x_test): 
      dists[i][j] = torch.sum((train_i - test_j)**2)
  '''
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train): 
    x_train_vec = x_train_flat[i]
    temp_dist = torch.sum( (x_test_flat - x_train_vec)**2, dim = 1)
    dists[i] = temp_dist
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  '''
  This was my original answer. It works but is too slow (too much reshaping?)

  x_train_flat = x_train.view(num_train, 1, -1)
  x_test_flat = x_test.view(1, num_test, -1)
  dists = torch.sum( (x_train_flat - x_test_flat)**2, dim=2)
  '''
  # use (x1 - x2)^2 = x1^2 - 2(x1)(x2) + x2^2
  x_train_flat = x_train.reshape(num_train, -1)
  x_test_flat = x_test.reshape(num_test, -1)

  x1_sqrd = torch.sum(x_train_flat**2, dim = 1).reshape(-1, 1)
  x2_sqrd = torch.sum(x_test_flat**2, dim =1).reshape(1, -1)
  dists = x1_sqrd + x2_sqrd  # x1^2 + x2^2
  dists += -2*( torch.mm( x_train_flat, x_test_flat.t()) )  # -2(x1)(x2)
...
def predict_labels(dists, y_train, k=1):
  ...
  # Get indices of the closest (smallest) values
  top_k, k_indcs = torch.topk(dists, k = k, dim = 0, largest = False)
  # Stores y labels for smallest values in top_k
  top_k_labls = torch.zeros( (num_test, k), dtype=torch.int64)
  k_indcs = torch.transpose(k_indcs, 0, 1)
  k_indcs_ls = k_indcs.tolist()
  # Assign y labels for each column in top_k
  for i in range(num_test): 
    top_k_labls[i] = y_train[k_indcs_ls[i]]
  top_k_labls = torch.transpose( top_k_labls, 0, 1)
  # Get mode of y_predictions
  y_pred, y_pred_indcs = torch.mode(top_k_labls, dim=0)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  # create empty list in dictionary for now to store k vals for each fold
  for i in range(len(k_choices)): 
    k_to_accuracies[k_choices[i]] = []

  print(k_to_accuracies)
  
  for i in range(num_folds): 
    # separate one fold for testing
    x_test_cur = x_train_folds.pop(0)
    y_test_cur = y_train_folds.pop(0)
    # combine other folds for training
    x_train_cur = torch.cat(x_train_folds)
    y_train_cur = torch.cat(y_train_folds)
    # train model
    KnnModel = KnnClassifier(x_train_cur, y_train_cur)

    # get accuracy values for each k value for current fold
    for j in range(len(k_choices)): 
      cur_k = k_choices[j]
      k_to_accuracies[cur_k].append( KnnModel.check_accuracy(x_test_cur, y_test_cur, k=cur_k, quiet=True) )

    # re-attach test data to the end so they naturally cycle (FIFO)
    x_train_folds.append(x_test_cur)
    y_train_folds.append(y_test_cur)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies: 
    ...
    k_mean = torch.mean(k_tensor).item()
    if k_mean > best_k_val: 
      best_k_val = k_mean
      best_k = k
  ##############################################################################
  #                            END OF YOUR CODE                                #
