...
def create_tensor_of_pi(M: int, N: int) -> Tensor:
    ...
    # can replace 3.14 with "torch.pi" for more accurate pi approximation
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    first_multiple_of_ten = start + (10 - (start % 10))
    # at least one multiple of ten exists
    if first_multiple_of_ten <= stop: 
      x = torch.arange(first_multiple_of_ten, stop, 10, dtype=dtype)
    # no multiples of ten exist --> return empty tensor
    else: 
      x = torch.tensor([], dtype=dtype); 
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    even_rows_odd_cols = x[0:num_rows:2, 1:num_cols:2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    M, N = x.shape
    y = x.clone()
    min_idxs = y.argmin(dim=1)
    y[[range(M), min_idxs]] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for i in range(x.shape[0]): 
      mat1 = x[i] 
      mat2 = y[i]
      batched_muls.append(torch.mm(mat1, mat2))
    z = torch.stack(tuple(batched_muls))
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    #                      TODO: Implement this function                     #
    ##########################################################################
    col_means = x.mean(dim=0).view(1, -1)
    col_std = x.std(dim=0).view(1, -1)
    y = (x - col_means) / col_std
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
